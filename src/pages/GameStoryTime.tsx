...
export default function GameStoryTime() {
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {story.highlightedWords.map((w) => (
            <span key={w} className="rounded-full border bg-white px-3 py-1">
              {w}
            </span>
          ))}
        </div>
  ...
  if (line < story.lines.length) {
    const words = story.highlightedWords.filter((w) =>
      story.lines[line].toLowerCase().includes(w.toLowerCase())
    );
    return (
      <div className="space-y-3">
